...
public:
	...
	void insert() {
		...
		// Cetak apakah antrian penuh
		if ((FRONT == 0 && REAR == max - 1) || (FRONT == REAR + 1)) {
			cout << "\nQueue overflow";
			return;
		}

		// Cetak apakah antrian kosong
		if (FRONT == -1) {
			FRONT = 0;
			REAR = 0;
		}
	...
	void remove() {
		...
		// Cek jika anttrian hanya memiliki satu elemen
		if (FRONT == REAR) {
			FRONT = -1;
			REAR = -1;
		}
	...
	void display() {
		...
		if (FRONT_position <= REAR_position) {
			...
			// Jika FRONT_positiion > REAR_position iterasi dari FRONT hingga akhir array
